...
{
    public class vApplicationsController : Controller
    {
        ...
        {
            List<tstproduct> tProd = tstdb.tstproducts.Where(x => x.OwnerId != 0).ToList();
            ViewBag.ProductsList = tProd;
            return View();
        }
        ...
        //[HttpGet]
        public JsonResult AddEditApp(int? id)
        {
            return Json(tstdb.tstapplications.Where(x => x.Id == id), JsonRequestBehavior.AllowGet);
        }

        [HttpPost]
        public JsonResult AddEditApp(tstapplication app)
        {
            app.OwnerId = 100;
        ...
        {
            List<tstapplications_developers> tApp = tstdb.tstapplications_developers.Where(x => x.OwnerId != 0).ToList();
        ...
        {
            List<tstapplications_searches> tApp = tstdb.tstapplications_searches.Where(x => x.OwnerId != 0).ToList();
